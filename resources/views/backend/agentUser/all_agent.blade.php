...
@section('admin') 
			...
			<div class="page-content">
				...
				<nav class="page-breadcrumb">
					<ol class="breadcrumb">
						 <a href="{{route('add.propertie')}}" class="btn btn-inverse-info">Add Agent</a>
					</ol>
				</nav>
                  ...
                  <table id="dataTableExample" class="table">
                    ...
                    @foreach($allAgent as $key => $item)  
                      <tr>
                        <td>{{$key+1}}</td>
                        <td>
                            <img src="{{(!empty($item->photo)) ? url('uploads/agent_images/'.$item->photo) : url('uploads/no_image.jpg')}}" alt=""style="width:70px; height:40px;"> 
                        </td>
                        <td>{{$item->name}}</td>
                        <td>{{$item->role}}</td>
